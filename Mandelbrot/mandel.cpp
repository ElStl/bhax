...
using namespace png;
#define SIZE 600
#define ITER_LIMIT 32000

        void mandel (int kepadat[SIZE][SIZE]) {
            ...
            int szelesseg = SIZE, magassag = SIZE, iteraciosHatar = ITER_LIMIT;
        ...
        main (int argc, char *argv[])
        {
            ...
            if (argc != 2)
            {
                cout << "Hasznalat: ./'programnev' 'fajlnev'"<<'\n';
                return -1;
            }

            int kepadat[SIZE][SIZE];

            mandel(kepadat);

            image < rgb_pixel > kep (SIZE, SIZE);
